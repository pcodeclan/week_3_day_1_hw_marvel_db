...
UPDATE people SET name = ('Krusty the Clown') WHERE name = ('Crusty the Clown'); --Q3

DELETE FROM movies WHERE (title) = ('Batman Begins'); --Q5
INSERT INTO people (name) VALUES ('Bart Simpson'); -- Q6
DELETE FROM people WHERE (name) = ('Eric Cartman'); -- Q7
INSERT INTO movies (title, year, show_time)
VALUES ('Avengers: Infinity War', 2018, '00:00'); -- Q8

UPDATE movies SET show_time = ('20:45') WHERE (title) = ('Iron Man 3'); --Q9 -- UPDATE does not allow brackets () in column names, when only one column is being selected.
DELETE FROM movies WHERE (year) = (2017); -- Extension

SELECT * FROM movies; --Q1
SELECT (name) FROM people; --Q2

SELECT FROM people WHERE (name) = ('Homer Simpson'); --Q4
SELECT (show_time) FROM movies WHERE (title) = ('Iron Man 2');--Q9
